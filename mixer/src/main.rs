...
const MONITORING_SIGNAL_SUBJECT: &str = "system.signal";
...
async fn publish_monitoring_signal(
    ...
    if let Ok(json_str) = serde_json::to_string(&payload) {
        let _ = client
            .publish(MONITORING_SIGNAL_SUBJECT, json_str.into())
            .await;
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mixer_instance_id = format!("mixer:{}", mixer_id);
    // EdgeからアクセスできるIPアドレスを指定します（ローカルテストなら127.0.0.1など）
    let mixer_addr = format!("127.0.0.1:{}", PORT);

    let nats_for_hb = nats_client.clone();
    let nats_for_signal = nats_client.clone();
    let nats_for_sub = nats_client.clone();
    let mixer_id_for_hb = mixer_id.clone();
    let mixer_addr_for_hb = mixer_addr.clone();
    let signal_instance_id = mixer_instance_id.clone();
    let signal_addr = mixer_addr.clone();
    let signal_publisher = nats_client.clone();
    let mixer_id_for_signal = mixer_id.clone();
    let signal_monitor = tokio::spawn(async move {
        let mut subscriber = match nats_for_signal.subscribe(MONITORING_SIGNAL_SUBJECT).await {
            Ok(sub) => sub,
            Err(e) => {
                eprintln!("[Mixer] 監視信号のサブスクライブに失敗しました: {}", e);
                return;
            }
        };
        ...
        while let Some(msg) = subscriber.next().await {
            if let Ok(json_str) = std::str::from_utf8(&msg.payload) {
                if let Ok(signal) = serde_json::from_str::<MonitoringSignal>(json_str) {
                    if signal.source == "orchestrator" && signal.event == "booted" {
                        publish_monitoring_signal(
                            &signal_publisher,
                            "mixer",
                            "booted",
                            &signal_instance_id,
                            "info",
                            format!("Mixer {} が起動しました。", mixer_id_for_signal),
                            json!({
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let payload = HeartbeatPayload {
                id: mixer_id_for_hb.clone(),
                addr: mixer_addr_for_hb.clone(),
                load: 10,
            };
    ...
    publish_monitoring_signal(
        ...
        &mixer_instance_id,
        "info",
        format!("Mixer {} が起動しました。", mixer_id.clone()),
        json!({
    ...
    tokio::select! {
        _ = tokio::signal::ctrl_c() => {
            println!("[Mixer] Ctrl-C を受信しました。");
        }
        _ = sigterm.recv() => {
            println!("[Mixer] SIGTERM を受信しました。");
        }
    }
