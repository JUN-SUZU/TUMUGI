...
- `monitoring.guildId`: 障害通知を送る Discord ギルドの ID
- `monitoring.channelId`: 障害通知を送る Discord チャンネルの ID
...
- Orchestrator, Mixer, Bot は `system.signal` に起動・停止・異常終了を送信します。
- Bot は `config.json` の `monitoring.guildId` と `monitoring.channelId` を使って、監視信号を Discord の指定チャンネルへ転送します。
- Orchestrator は Mixer の再起動信号を受けると、Redis に残っている Union 状態を Mixer へ再送します。
- Mixer は Orchestrator の起動信号を受けると、自分の起動完了を再通知します。
...
- Orchestrator が応答しない場合は、NATS の接続先設定と、監視チャンネルの設定を確認してください。
